...
class ContestantModel {
	...
	public static function CHANGE($id){
		...
		$sql = "select * from contestants where contestantid = $id";
		$res = $mysqli->query($sql);

		//fetching data
		$cols = $res->fetch_assoc();

		//fetching contestant type in database
		$contype = $cols['contestanttype'];

		//condition for contestant type
		if ($contype == "qualified") {
			$stmt = $mysqli->prepare('UPDATE contestants set contestanttype = "disqualified" WHERE contestantid =?');
		}elseif ($contype == "disqualified") {
			$stmt = $mysqli->prepare('UPDATE contestants set contestanttype = "qualified" WHERE contestantid =?');
		}
