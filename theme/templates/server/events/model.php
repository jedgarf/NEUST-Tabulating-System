...
class EventModel {
	...
	public static function create($data){
		...
		if ($mysqli->connect_errno) {
			...
			$act = $result2->fetch_assoc();
			$actdatestart = $act['actstartdate'];
			$actdateend = $act['actenddate'];
			...
			if(count($querydata) > 0){
				...
				if ($eventdate < $actdateend && $eventdate > $actdatestart) {
					if ($stmt = $mysqli->prepare('INSERT INTO events (eventname,eventdescription,eventdate,actid) VALUES (?,?,?,?)')){
						...
						return print json_encode(array('success' =>false,'status'=>500,'msg' =>'Error message: Invalid Date!'));
				}
			}
	...
	public static function read(){
		...
		if ($mysqli->connect_errno) {
			...
			//session_start();
			//$usid = $_SESSION['user'];
			//$query1 ="SELECT * FROM events where eventid = (select eventid from users where userid = $usid)";
	...
	public static function detail($id){
		...
		if ($mysqli->connect_errno) {
			...
			//$query = "SELECT eventid, eventname, eventdescription, eventdate FROM events";
