...
function loadcandidateTojudgeCombo(){
    $("#candidate4judge").html('');
    //$("#candidate4judge").html('<option value=""></option>');
    console.log('>loading data to combo-judge after clearing..');
    $.ajax({
        url: '../server/contestants_Ext3/index.php',
        async: false,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            ...
            if (decode) {
                if (decode.childs.length > 0) {
                    for (var i = 0; i < decode.childs.length; i++) {
                        var row = decode.childs; 
                        var html = '<option value="'+row[i].contestantid+'">'+row[i].name+'fgdf</option>';
                        $("#candidate4judge").append(html);
                    }  
                }
            }
        },
...
function clear(){
    $('.candidate4judge .select2-choice .select2-chosen').html('');
}
...
function savescore(fieldId){
    ...
    if(action=='Save'){
        ...
        if ($('#event4judge').val() == '') {
            $('#event4judge').next('span').text('Event Name is required!');
            empty = true;
        }

        if ($('#candidate4judge').val() == '') {
            $('#candidate4judge').next('span').text('Candidate Name is required!');
            empty = true;
        }
        if (score == '') {
            empty = true;
       ...
       // }
        ...
        $.ajax({
                ...
                dataType: 'json',
                data: {
                    eventid: $('#event4judge').val(),
                   // judgeid: judgeid,
                    criteriaid: criteriaid,
                    contestantid:$('#candidate4judge').val(),
                    score:score
                },
                success: function(response) {
                    var decode = response;
